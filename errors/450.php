...
class view_error extends common
{
    ...
    {
        global $conf;
        $this->set_title('450  - Blocked by Windows Parental Controls ');
        $this->set_template_file(make_path('template',"httperror",'html',false));
        $this->set_main("<h1>Blocked by Windows Parental Controls </h1><br/>Code défini par Microsoft. Cette erreur est produite lorsque les outils de contrôle parental de Windows sont activés et bloquent l’accès à la page.<br/><br/><a href='".$conf->main_url_root."'>Retour à l'index</a>".(isset($_SERVER['HTTP_REFERER'])?"<br/><a href='".$_SERVER['HTTP_REFERER']."'>Retour à la page précédente</a>":"")."");
        echo $this->gen_page();
        exit;
    }
}
