...
class view_error extends common
{
    ...
    {
        global $conf;
        $this->set_title('416  - Requested range unsatisfiable ');
        $this->set_template_file(make_path('template',"httperror",'html',false));
        $this->set_main("<h1>Requested range unsatisfiable </h1><br/>Champs d’en-tête de requête « range » incorrect.<br/><br/><a href='".$conf->main_url_root."'>Retour à l'index</a>".(isset($_SERVER['HTTP_REFERER'])?"<br/><a href='".$_SERVER['HTTP_REFERER']."'>Retour à la page précédente</a>":"")."");
        echo $this->gen_page();
        exit;
    }
}
