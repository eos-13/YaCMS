...
class view_error extends common
{
    ...
    {
        global $conf;
        $this->set_title('510  - Not extended ');
        $this->set_template_file(make_path('template',"httperror",'html',false));
        $this->set_main("<h1>Not extended </h1><br/>RFC 2774 : la requête ne respecte pas la politique d'accès aux ressources HTTP étendues.<br/><br/><a href='".$conf->main_url_root."'>Retour à l'index</a>".(isset($_SERVER['HTTP_REFERER'])?"<br/><a href='".$_SERVER['HTTP_REFERER']."'>Retour à la page précédente</a>":"")."");
        echo $this->gen_page();
        exit;
    }
}
