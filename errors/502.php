...
class view_error extends common
{
    ...
    {
        global $conf;
        $this->set_title('502  - Bad Gateway ou Proxy Error ');
        $this->set_template_file(make_path('template',"httperror",'html',false));
        $this->set_main("<h1>Bad Gateway ou Proxy Error </h1><br/>Mauvaise réponse envoyée à un serveur intermédiaire par un autre serveur.<br/><br/><a href='".$conf->main_url_root."'>Retour à l'index</a>".(isset($_SERVER['HTTP_REFERER'])?"<br/><a href='".$_SERVER['HTTP_REFERER']."'>Retour à la page précédente</a>":"")."");
        echo $this->gen_page();
        exit;
    }
}
