...
class view_error extends common
{
    ...
    {
        global $conf;
        $this->set_title('428  - Precondition Required ');
        $this->set_template_file(make_path('template',"httperror",'html',false));
        $this->set_main("<h1>Precondition Required </h1><br/>(RFC 6585) La requête doit être conditionnelle<br/><br/><a href='".$conf->main_url_root."'>Retour à l'index</a>".(isset($_SERVER['HTTP_REFERER'])?"<br/><a href='".$_SERVER['HTTP_REFERER']."'>Retour à la page précédente</a>":"")."");
        echo $this->gen_page();
        exit;
    }
}
