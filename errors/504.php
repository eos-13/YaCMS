...
class view_error extends common
{
    ...
    {
        global $conf;
        $this->set_title('504  - Gateway Time-out ');
        $this->set_template_file(make_path('template',"httperror",'html',false));
        $this->set_main("<h1>Gateway Time-out </h1><br/>Temps d’attente d’une réponse d’un serveur à un serveur intermédiaire écoulé<br/><br/><a href='".$conf->main_url_root."'>Retour à l'index</a>".(isset($_SERVER['HTTP_REFERER'])?"<br/><a href='".$_SERVER['HTTP_REFERER']."'>Retour à la page précédente</a>":"")."");
        echo $this->gen_page();
        exit;
    }
}
