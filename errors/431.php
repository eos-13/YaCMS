...
class view_error extends common
{
    ...
    {
        global $conf;
        $this->set_title('431  - Request Header Fields Too Large ');
        $this->set_template_file(make_path('template',"httperror",'html',false));
        $this->set_main("<h1>Request Header Fields Too Large </h1><br/>(RFC 6585) Les entêtes HTTP émises dépassent la taille maximale admise par le serveur<br/><br/><a href='".$conf->main_url_root."'>Retour à l'index</a>".(isset($_SERVER['HTTP_REFERER'])?"<br/><a href='".$_SERVER['HTTP_REFERER']."'>Retour à la page précédente</a>":"")."");
        echo $this->gen_page();
        exit;
    }
}
