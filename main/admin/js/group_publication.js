...
function valideDoubleName(val,colId,col)
{
    ...
    jQuery.ajax({
        url: "group_publication?action=valid",
        data: data,
        type: "POST",
        dataType: "json",
        async: false,
...
function showChildGrid(parentRowID, parentRowKey)
{
    jQuery.ajax({
        ...
        success: function (a) {
            ...
            {
                jQuery('#'+parentRowID).append('<tr><td><div style="display: inline; margin-right:10px;min-width:150px;width:150px;max-width:150px;">'+obj[i].name+' '+obj[i].firstname+'</div></td><td><button onClick="toggle_approuve('+parentRowKey+','+obj[i].id+','+obj[i].can_approuve+')">'+ (obj[i].can_approuve==1?i18n.translate("Peut approuvé").fetch():i18n.translate("Ne peut pas approuvé").fetch()) +'</button></td><td><button onClick="remove_member('+parentRowKey+','+obj[i].id+')">'+i18n.translate("Remove").fetch()+'</button></td></tr>');
            }
...
jQuery(document).ready(function(){
    jQuery('#jqGrid').jqGrid({
        url: 'group_publication?action=list',
        mtype: 'POST',
        datatype: 'json',
        colModel: [
                  ...
                  viewrecords: true,
                  editurl: 'group_publication?action=edit',
                  height: 550,
                  shrinkToFit: true,
                  autowidth:true,
                  rowNum: 20,
                  ...
                  hoverrows: true,
                  altRows: true,
                  caption: i18n.translate('Groupe de publication').fetch(),
                  pager: '#jqGridPager',
...
function remove_member(gid,uid)
{
    ...
    var f = jQuery('<form>').prop('action','group_publication').prop('method','post').prop('id','tmpForm')
                            .append(g)
                            .append(a)
                            .append(u);
...
function toggle_approuve(gid,uid,can_approuve)
{
    if (can_approuve == 1) can_approuve = 0;
    else if (can_approuve == 0) can_approuve = 1;
    var g = jQuery('<input>').prop('type','hidden').prop('name','gid').val(gid);
    var u = jQuery('<input>').prop('type','hidden').prop('name','uid').val(uid);
    var c = jQuery('<input>').prop('type','hidden').prop('name','can_approuve').val(can_approuve);
    var a = jQuery('<input>').prop('type','hidden').prop('name','action').val('toggle_approuve');
    var f = jQuery('<form>').prop('action','group_publication').prop('method','post').prop('id','tmpForm')
                            .append(g)
                            .append(a)
                            .append(c)
