...
$.fn.editInPlace.defaults = {
    ...
    field_type:            "text", // string: "text", "textarea", "bool", or "select";  The type of form field that will appear on instantiation
    ...
    textarea_rows:        30, // integer: set rows attribute of textarea, if field_type is set to textarea. Use CSS if possible though
    textarea_cols:        75, // integer: set cols attribute of textarea, if field_type is set to textarea. Use CSS if possible though
...
$.extend(InlineEditor.prototype, {
    ...
    createEditorElement: function() {
        if (-1 === $.inArray(this.settings.field_type, ['text', 'textarea', 'select','bool']))
            throw "Unknown field_type <fnord>, supported are 'text', 'textarea', 'bool' and 'select'";
        ...
        else if ("bool" === this.settings.field_type){
            var on_txt = this.settings.checked_on;
            var off_txt = this.settings.checked_off;
            editor = $('<input type="checkbox" ' + this.inputNameAndClass()
                + ' />').change(function()
                        {
                            if (jQuery(this).is(':checked'))
                            {
                                jQuery(this).val(on_txt);
                            } else {
                                jQuery(this).val(off_txt);
                            }
                        });
    ...
    setInitialValue: function() {
        ...
        if (this.settings.field_type == 'bool'){
            if (initialValue == this.settings.checked_on){
