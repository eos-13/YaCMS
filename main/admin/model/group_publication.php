<?php
class model_group_publication extends admin_common
{
    public $id;
    public $responce;
    ...
    public function add_members($post)
    {
        ...
        if (is_array($post['add_groups']))
        {
            foreach($post['add_groups'] as $key => $val)
            {
                ...
                {
                    $requete = "INSERT INTO group_publication_user
                                            (group_publication_refid, user_refid)
                                     VALUES (".$val.",".$val1.")";
        ...
        if (isset($post['rem_group']))
        {
            foreach($post['rem_members'] as $key => $val)
            {
                $requete = "DELETE FROM group_publication_user
                                  WHERE group_publication_refid = ".$post['rem_group']."
                                    AND user_refid = ".$val." ";
                $sql = $this->db->query($requete);
                if (!$sql) $error = true;
            }
    ...
    public function list_members($gid)
    {
        ...
        $requete = "SELECT user.id,
                           user.name,
                           user.firstname,
                           group_publication_user.can_approuve
                      FROM user,
                           group_publication_user
                     WHERE group_publication_user.user_refid = user.id
                       AND group_publication_user.group_publication_refid = ".$gid."
        ...
        while($res = $this->db->fetch_object($sql))
        {
            $a[]=array( "id" => $res->id,
                    ...
                    "can_approuve" => $res->can_approuve
             );
        }
        return $a;
    }
    public function toggle_approuve($post)
    {
        $requete = "UPDATE `group_publication_user`
                       SET `can_approuve` = ".$post['can_approuve'] ."
                     WHERE user_refid = ".$post['uid']."
                       AND group_publication_refid =  ".$post['gid'];
        $sql = $this->db->query($requete);
        if ($sql) $this->message = _("Opération effectuée");
        else $this->message = _("Opération echouée");
        return $sql;
    ...
    public function list_group_publication()
    {
        return $this->jqGrid();
    }
    public function valid_group_publication($post)
    {
    ...
    public function edit_group_publication($post)
    {
        load_alternative_class('class/group_publication.class.php');
        $u = new group_publication($this->db);
        $this->db->begin();
        if ($post['oper']=='add')
        {
            $id = $u->create();
