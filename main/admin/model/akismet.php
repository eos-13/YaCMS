<?php
class model_akismet extends admin_common
{
    public $id;
    public $responce;
    ...
    public function run()
    {
        ...
        $this->check_key = $c->verify_akismet_key();
        return (true);
    }
    ...
    public function edit($post)
    {
        ...
        if ($post['action'] == 'spam')
        {
            $ret = $c->send_as_spam(array('author'=> $c->get_author_html , 'comment' => $c->get_content() ));
            if($ret)
                $error = $c->del();
    ...
    public function detail($post)
    {
        ...
        if ($c->fetch($post['id']))
        {
            ...
            $html .= "<tr><th valign=top>akismetaire</th><td>".$c->get_content()."</td></tr>";
            $html .= "</table>";
            return $html;
        } else {
