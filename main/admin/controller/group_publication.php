<?php
class controller_group_publication extends admin_common
{
    private $id;
    private $model;
    ...
    private function action($action,$post)
    {
        ...
        switch($action)
        {
            case "list":
            {
                ...
                $res = $this->model->list_group_publication();
            ...
            {
                $this->log->log(get_class($this). ' Uid: ' .$current_user->get_id(). ' Action '.$action. " Params: ".print_r($_REQUEST,1),LOG_DEBUG);
                $res = $this->model->edit_group_publication($post);
                $this->message = $this->model->message;
                return $res;
            }
            break;
            ...
            {
                $this->log->log(get_class($this). ' Uid: ' .$current_user->get_id(). ' Action '.$action. " Params: ".print_r($_REQUEST,1),LOG_DEBUG);
                $res = $this->model->valid_group_publication($post);
                $this->message = $this->model->message;
                return $res;
            }
            break;
            ...
            {
                $this->log->log(get_class($this). ' Uid: ' .$current_user->get_id(). ' Action '.$action. " Params: ".print_r($_REQUEST,1),LOG_DEBUG);
                $res = $this->model->add_members($post);
                $this->message = $this->model->message;
                return $res;
            }
            break;
            ...
            {
                $this->log->log(get_class($this). ' Uid: ' .$current_user->get_id(). ' Action '.$action. " Params: ".print_r($_REQUEST,1),LOG_DEBUG);
                $res =  json_encode($this->model->list_members($post['id']));
                $this->message = $this->model->message;
                return $res;
            }
            break;
            case "toggle_approuve":
            {
                $this->log->log(get_class($this). ' Uid: ' .$current_user->get_id(). ' Action '.$action. " Params: ".print_r($_REQUEST,1),LOG_DEBUG);
                $this->model->toggle_approuve($post);
                $this->message = $this->model->message;
            }
            break;
        }
