...
load_alternative_class('class/preview.class.php');
class view_preview extends preview
{
    private $templ;
    private $id;
    protected $page;
    public function run()
    {
        if ($_REQUEST['type']=="rev")
        {
            load_alternative_class('class/revision.class.php');
            $this->id = $_REQUEST['id'];
            $this->page = new revision($this->db);
            $this->page->fetch($this->id);
            $this->page->get_all();
        } else {
        ...
        if ($this->page->get_model_refid() > 0)
        {
            ...
            if ($fiche)
            {
                if( isset($_REQUEST['save_preview']) &&  $_REQUEST['save_preview'] == 1 && !$_REQUEST['type'] )
                {
                    global $current_user;
                    $requete = "INSERT INTO html_save
                                            (html,page_refid,save_by)
                                     VALUES ('".addslashes($fiche)."','".$this->page->get_id()."','".$current_user->get_id()."')";
        ...
        if ($this->page->get_extra_params() && $this->page->get_extra_params()."x" != "x")
        {
            ...
            foreach($data as $key=>$val)
            {
                ...
                if (isset($data->$cache) && $data->$cache == 1)
                {
                    ...
                    $regex = false;
                    if (preg_match('/^image/',$val))
                    {
                        $regex = "^.+\.(png|jpeg|gif|jpg)$";
                    } elseif (preg_match("/^video/",$val))
                    {
                        $regex = "^.+\.(mp4|ogg)$";
                    }
    ...
    private function template_bdd()
    {
        ...
        $this->set_title("Preview " .$this->page->get_title());
