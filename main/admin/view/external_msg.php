...
class view_external_msg extends admin_common
{
    public $message;

    private function make_editable_jqgrid_list($jsVarName='list')
    {
    ...
    public function run()
    {
        $this->set_title(_('Messages externes'));
        ...
        $this->set_js_code(
                $this->make_editable_jqgrid_list('listAllStatus','external_msg_status','name','name')
        );
        $this->set_js_code(
                $this->make_editable_jqgrid_list('listAllType','external_msg_type','name','name')
        );
        global $lang,$conf;
        $tmpLang = substr($lang,0,2);
