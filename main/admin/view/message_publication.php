<?php
class view_message_publication extends admin_common
{
    public $id;
    ...
    private function set_tinymce_vars($p)
    {
        ...
        $tmp .= join(',',$tmpA);
        $tmp .='";';
        $tmp .= 'var content_css_set="'.make_path("css","style","css",false,true).'";';
        global $conf;
    ...
    public function run($content,$property,$allgroup_publi,$load=0)
    {
        $this->set_css(array('admin',$this->req,'jquery-ui.min',"jquery.growl",'jquery-ui-multiselect'));
        $this->set_js(array('tinymce_var','admin', 'jquery','jquery-ui.min','tinymce/tinymce.min','tinymce/jquery.tinymce.min','jquery-ui-multiselect',"jquery.growl",$this->req));

        $this->set_title(_('Message de publication'));
        $this->set_js(array (
        ...
        $this->set_tinymce_vars((isset($property['css']['value'])?$property['css']['value']:false));
        if ($property)
        {
        ...
        $this->set_extra_render("allgroup_publi",$allgroup_publi);
